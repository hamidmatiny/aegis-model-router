...
description: Open a PR against hamidmatiny/aegis for model-router only — never merge
user-invocable: true
---

# Open Component PR

**Hard stops:** no merge, no force-push to main, no prod credentials, no edits outside `model-router/` (and explicitly co-owned shared policy files).

1. `/audit-repo-access` must show scoped `AEGIS_PR_TOKEN` (not too_broad). If absent/too_broad — stop.
2. Branch from latest main: `component/model-router/<short-slug>`.
